...
module.exports = {
  add: async (req, res) => {
    ...
    const checkCode = await FeeItem.findOne({ code: params.code });
  ...
  edit: async (req, res) => {
    ...
    //CHECK DUPLICATE CODE
    const checkCode = await FeeItem.findOne({ id: {'!=' : params.id}, code: params.code });
    if (checkCode) {
  ...
  search: async (req, res) => {
    ...
    for (let feeItem of arrObjFeeItems) {
      ...
      let formattedCurrency = await sails.helpers.currencyFormat(feeItem.amount, currency);
      tmpData.formattedAmount = formattedCurrency;
      tmpData.tool = await sails.helpers.renderRowAction(feeItem);

...
class ListIndexFoodBackendEKP {
  ...
  handleItemActions() {
    ...
    _this.tableObj.on('click', '.edit-row', function (e) {
      ...
      //get AJAX data
      Cloud.getFood.with({ id: id }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
        if (err) {
          console.log(err);
          return;
        }
  ...
  initSweetAlert(id) {
    swal({
      ...
      if (value) {
        //get AJAX data
        Cloud.trashFood.with({ ids: id }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
          if (err) {
            console.log(err);
            return;
  ...
  //END TRASH
	...
	initSwitchStatus() {
        ...
        Cloud.switchStatusFood.with({ id: id }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
					if (err) {
						console.log(err);
						return;
					} else if (responseBody) {

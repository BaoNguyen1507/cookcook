class IndexSubjectBackendEKP extends BaseBackendEKP {
  constructor() {
    super();
    this.initialize();
  }
  ...
  initialize() {
    ...
    this.form = new FormIndexSubjectBackendEKP(this);
    this.list = new ListIndexSubjectBackendEKP(this);

  }
}

class FormIndexSubjectBackendEKP {
  constructor(opts) {
    _.extend(this, opts);
    this.formId = 'formSubject';
    this.formObj = $('#' + this.formId);
    //this.headline = this.formObj.closest('.card').find('.card-title');
    this.headline = $('.modal-title');
    ...
    _this.formObj.formValidation({
      ...
      .on('success.form.fv', function (e) {
        ...
        if (manner === 'edit') {
          ...
          Cloud.editSubject.with(tmpData).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
            if (err) {
          ...
          Cloud.addSubject.with(tmpData).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
            if (err) {
              _this.alert.removeClass('d-none').addClass("alert-danger").html(_this.messages.error);
              setTimeout(function(){
                _this.alert.removeClass('alert-danger').addClass("d-none");
...
class ListIndexSubjectBackendEKP {
  constructor(opts) {
    _.extend(this, opts);

    this.langUrl = this.lang == 'en' ? '' : '/js/backend/datatable.json';
    this.tblId = 'tblSubject';
  ...
  initDataTable() {
    ...
    var table = this.tableObj.DataTable({
      ...
      "processing": true,
      "serverSide": true,
      "ajax": "/api/v1/backend/subject/search?status=" + params.status,
  ...
  handleItemActions() {
    ...
    _this.tableObj.on('click', '.edit-row', function (e) {
      ...
      //get AJAX data
      Cloud.getSubject.with({ id: id }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
        if (err) {
          console.log(err);
          return;
        }
  ...
  initSweetAlert(id) {
    swal({
      ...
      if (value) {
        //get AJAX data
        Cloud.trashSubject.with({ ids: id }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
          if (err) {
            console.log(err);
            return;
  ...
  //END ALERT AND TRASH
	...
	initSwitchStatus() {
        ...
        Cloud.switchStatusSubject.with({ id: id }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
					if (err) {
						console.log(err);
						return;
					} else if (responseBody) {

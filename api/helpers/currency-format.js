module.exports = {
  ...
  fn: async function (inputs, exits) {
    ...
    let result = (Math.round(inputs.number * inputs.currency.convertRate * 1000) / 1000).toFixed(inputs.currency.decimalPlaces).replace('.', inputs.currency.decimalPoint).replace(/\B(?=(\d{3})+(?!\d))/g, inputs.currency.numberSeperatorSymbol);

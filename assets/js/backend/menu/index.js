class IndexListMenuBackendEKP extends BaseBackendEKP {
  ...
  initCalendar() {
    ...
    let startTimeCourseSession = _this.modalMenu.attr('data-starttime');
    let endTimeCourseSession = _this.modalMenu.attr('data-endtime');
    let startCourseSession = moment.tz(startTimeCourseSession + " 00:00", "Asia/Ho_Chi_Minh");
    let endCourseSession = moment.tz(endTimeCourseSession + " 00:00", "Asia/Ho_Chi_Minh");
    ...
    if (_this.calendar.length) {
      _this.calendar.fullCalendar({
        ...
        dayNames: [ _i18nMsg['Monday'],_i18nMsg['Tuesday'],_i18nMsg['Wednesday'],_i18nMsg['Thursday'],_i18nMsg['Friday'],_i18nMsg['Saturday'],_i18nMsg['Sunday']],
        dayNamesShort: [ _i18nMsg['Mon'],_i18nMsg['Tue'],_i18nMsg['Wed'],_i18nMsg['Thu'],_i18nMsg['Fri'],_i18nMsg['Sat'],_i18nMsg['Sun']],
        //height: 'parent',
        events: function (start, end, timezone, callback) {
          jQuery.ajax({
            ...
            data: {
              classId: classId,
              start: startCourseSession.format(),
              end: endCourseSession.format()
            },
          ...
          Cloud.getMenuByDateUse.with(params).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
            if (err) {
              ...
              if (!responseBody.menu.id ) {
                let _i18nMsg = i18n[_this.lang];
                swal(`${moment(date.format(), "YYYY-MM-DD").format("DD/MM/YYYY")} ` + _i18nMsg["have no menu. Please add new"], {
                  icon: "warning",
                  button: false,
                });
              } else {
                // _this.menuDetail.find('.dateMenu').html(currDate == moment(date.format(), "YYYY-MM-DD").format("DD/MM/YYYY") ? 'Hôm nay' : moment(date.format(), "YYYY-MM-DD").format("DD/MM/YYYY"));
                // // _this.menuDetail.find('#editMenu').html(`
                // //   <a href="/backend/menu/edit/${responseBody.id}" class="btn btn-secondary">Chỉnh sửa</a>
                // //   <a href="/backend/menu/add" class="btn btn-primary">Thêm mới</a>
                // // `);
                // $('#accordion').find('.card').remove();
                // $('.menuEmpty').remove();
                // $('#accordion').append(responseBody.html);
                // $('#singleDay').removeClass('hidden');
                // $('#multipleDay').addClass('hidden');
                // _this.modalMenu.attr('data-menuid', responseBody.id);
  ...
  initDeleteMenu() {
    ...
    $('#deleteMenu').on('click', () => {
      ...
      swal({
        ...
        if (value) {
          Cloud.deleteMenu.with({ id: id }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
            if (err) {
              console.log(err);
              return;

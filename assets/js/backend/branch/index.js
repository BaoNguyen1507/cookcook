...
class ListIndexBranchBackendEKP {
  ...
  initSweetAlert(id) {
    swal({
      ...
      if (value) {
        //get AJAX data
        Cloud.trashBranch.with({ ids: id }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
          if (err) {
            console.log(err);
            return;
  ...
  initTriggerAddBranch() {
	...
	initSwitchStatus() {
        ...
        Cloud.switchStatusBranch.with({ id: id }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
					if (err) {
						console.log(err);
						return;
					} else if (responseBody) {

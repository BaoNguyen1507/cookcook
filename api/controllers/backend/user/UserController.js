...
const BackendUserController = {
  ...
  */
  init: async function (req, res) {
    ...
    let admin = await UserService.get({ emailAddress: "admin@gmail.com" });
    if (admin) {
  ...
  add: async (req, res) => {
    ...
    let email = await UserService.get({ emailAddress: params.emailAddress });
    if (email) {
    ...
    // if (type) {
    //   await User.addToCollection(newObj.id, 'staff', type).exec(function (err) { });
    // }
    return res.ok(newObj);
  ...
  edit: async (req, res) => {
    ...
    if (params.metaFields && !Utils.isJsonString(params.metaFields)) return res.serverError(ErrorMessages.SYSTEM_JSON_FORMAT_FAIL);
    if (params.password != params.passwordConfirm) return res.badRequest(ErrorMessages.PASSWORD_IS_NOT_MATCH);
    let Lclass = (params.classes) ? params.classes : [];
    let data = {
      emailAddress: params.emailAddress,
      phone: params.phone,
  ...
  trash: async (req, res) => {
    ...
    let data2 = { status: sails.config.custom.STATUS.TRASH };
    let ids = params.ids;
    ...
    if (typeof (ids) == 'object') {
      for (var i = 0; i < ids.length; i++) {
        ...
        if (userObj) await User.destroy({ id: ids[i] });
        // let userObj = await UserService.get({ id: ids[i] });
      ...
      if (userObj) await User.destroy({ id: ids });
  ...
  search: async (req, res) => {
    ...
    if (params.userType) {
      where.$and.push({ userType: parseInt(params.userType) })
    }
    ...
    for (let userObj of arrObjUser) {
      let user = await UserService.get({ id: userObj.id });
      let tmpData = {};
    ...
    return res.ok({ draw: draw, recordsTotal: totalUser, recordsFiltered: totalUser, data: resUsers });
  },
  ...
  switchStatus: async (req, res) => {
    ...
    let userObj = await UserService.get({ id: params.id });
    if (!userObj) return res.badRequest(ErrorMessages.USER_NOT_FOUND);

    //switch status of current obj
    if (userObj.status == 1) userObj = await UserService.edit({ id: params.id }, { status: 0 });
    else userObj = await UserService.edit({ id: params.id }, { status: 1 });

...
class ListIndexParentBackendEKP {
  ...
  initSweetAlert(id) {
    swal({
      ...
      if (value) {
        Cloud.trashParent.with({ ids: id }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
          if (err) {
            console.log(err);
            return;
	...
	initSwitchStatus() {
        ...
        Cloud.switchStatusParent.with({ id: id }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
					if (err) {
						console.log(err);
						return;
					} else if (responseBody) {

...
module.exports = {
  ...
  add: async (req, res) => {
    ...
    //             .then((info) => {
    //               fileUpload[0].media_details.width = info.width;
    //               fileUpload[0].media_details.height = info.height;
    //             }).catch((err) => { sails.log(err); });
    //           fileUpload[0].media_details.path = '/uploads/' + moment().format('YYYY/MM') + '/' + destFileName;
    //         } else {
    //           Sharp(file.fd).resize(size.width, size.height)
    //             .toFile(require('path').resolve(uploadConfig.PATH_FOLDER, 'assets/uploads/') + '/' + moment().format('YYYY/MM') + '/' + destFileName)
    //             .then((info) => { }).catch((err) => { sails.log(err); });
    //           fileUpload[0].media_details.sizes[size.type] = {
    ...
    // if (fileUpload[0].media_details == null) {
    //   return res.badRequest(ErrorMessage.MEDIA_ERR_UPLOAD_FAIL);
    // }

    // // PREPARE DATA MEDIA
    // const newData = {
    //   title: params.title ? params.title : String(moment().valueOf()), // REQUIRED
    //   thumbnail: fileUpload[0].media_details,

...
class FormIndexCurrencyBackendEKP {
  constructor(opts) {
    ...
    this.inputSymbolRight = $('#symbolRight');
    this.inputConvertRate = $('#convertRate');
...
class ListIndexCurrencyBackendEKP {
  ...
  initSweetAlert(id) {
    swal({
      ...
      if (value) {
        //get AJAX data
        Cloud.trashCurrency.with({ ids: id }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
          if (err) {
            console.log(err);
            return;
  ...
  //END TRASH
	...
	initSwitchStatus() {
        ...
        Cloud.switchStatusCurrency.with({ id: id }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
					if (err) {
						console.log(err);
						return;
					} else if (responseBody) {

...
class ListIndexPostBackendEKP {
  ...
  initDataTable(_this) {
    ...
    var table = this.tableObj.DataTable({
      ...
      "processing": true,
      "serverSide": true,
      "ajax": "/api/v1/backend/post/search?status=" + params.status,
  ...
  initSweetAlert(id) {
    swal({
      ...
      if (value) {
        //get AJAX data
        Cloud.trashPost.with({ ids: id }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
          if (err) {
            console.log(err);
            return;
	...
	initSwitchStatus() {
        ...
        Cloud.switchStatusPost.with({ id: id }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
					if (err) {
						console.log(err);
						return;
					} else if (responseBody) {

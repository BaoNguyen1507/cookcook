...
class IndexListScheduleBackendEKP extends BaseBackendEKP {
  ...
  initCalendar() {
    let _this = this;
    let _i18nMsg = i18n[_this.lang];
    let startTimeCourseSession = $('#modalSchedule').attr('data-starttime');
    let endTimeCourseSession = $('#modalSchedule').attr('data-endtime');
  ...
  initDeleteSchedule() {
    ...
    $('#deleteSchedule').on('click', () => {
      ...
      swal({
        ...
        if (value) {
          Cloud.deleteSchedule.with({ id: id }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
            if (err) {
              console.log(err);
              return;

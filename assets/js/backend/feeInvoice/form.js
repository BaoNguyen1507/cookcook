  ...
  initDataTableStudent(classId, studentId) {
    ...
    table.on('xhr', function () {
      if (studentId == '') {
        ...
        $('#studentIdFormAdd').append(new Option('---', ''));
...
class ListFeeInvoiceBackendEKP {
  ...
  initSweetAlertDeleteFeeInvoice(id) {
    ...
    swal({
      ...
      if (value) {
        //get AJAX data
        Cloud.deleteFeeInvoice.with({ ids: id }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
          if (err) {
            console.log(err);
            return;
  ...
  initSweetAlertPublicFeeInvoice() {
    $('#btnPublicFeeInvoice').click(() => {
      ...
      swal({
        ...
        if (value) {
          //get AJAX data
          Cloud.publicFeeInvoice.with({ ids: _this.feeInvoiceIds }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
            if (err) {
              console.log(err);
              return;
            } else if (responseBody) {

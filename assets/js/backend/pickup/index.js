...
class ListIndexPickUpBackendEKP {
  ...
  initDataTable(dateChoosed) {
    ...
    var table = this.tableObj.DataTable({
      ...
      "serverSide": true,
      "ajax": {
        url:  `/api/v1/backend/class-${params.classId}/pickup?date=${params.date}&branchId=${params.branchId}`,
        async: true,
    ...
    $('#branchBtn').on('click', (e) => {
      ...
      _this.routingPrepare(branchID, classID);
    })
  }
  routingPrepare(branchId,classId) {
    let _this = this;
    const opts = [
      {'value':'/backend/pickup/filter?branchId=' + branchId + '&classId='+ classId, 'text':'Branch'}
    ];

...
module.exports = {
  ...
  list: async (req, res) => {
    ...
    const albums = await Album.find({ status: sails.config.custom.STATUS.ACTIVE, or: [{ classObj: null }, { classObj: params.classId }] }).limit(limit).skip((page - 1) * limit).sort([{ createdAt: 'DESC' }]);
    if (albums.length > 0) {

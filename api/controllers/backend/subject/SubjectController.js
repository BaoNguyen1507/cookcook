...
module.exports = {
  add: async (req, res) => {
    ...
    const checkCode = await Subject.findOne({ code: params.code });
    if (checkCode) return res.ok({
      message: ErrorMessages.SUBJECT_CODE_DUPLICATED.message
    });
  ...
  getMasterSubject: async (req, res) => {
  ...
  search: async (req, res) => {
    ...
    for (let subject of arrObjSubjects) {
      ...
      tmpData.tool = await sails.helpers.renderRowAction(subject);

...
module.exports = {
  ...
  fn: async function (inputs, exits) {
    ...
    let count = await User.count({ status: sails.config.custom.STATUS.ACTIVE, userType: sails.config.custom.TYPE.TEACHER });
    ...
    let teachers = await User.find({ status: sails.config.custom.STATUS.ACTIVE, userType: sails.config.custom.TYPE.TEACHER }).limit(limit).skip(limit * (_default.pageActive - 1));

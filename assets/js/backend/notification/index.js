            ...
            if (key === 'classList') {
              ...
              $('#message-section').html(`
                ...
                <div class="col-md-9">
                    ...
                    data-fv-notempty-message="<%= __('Required feild !') %>" >${value}</textarea>
                </div>
              `);
              _this.initEditorNotificationForm();
              ...
              if (value == -1) {
                _this.formObj.find('#typePrivate')[0].checked = true;
                _this.formObj.find('#typePublic')[0].checked = false;
              } else if (value == 0) {
                _this.formObj.find('#typePrivate')[0].checked = false;
                _this.formObj.find('#typePublic')[0].checked = true;
              }
            } else {
  ...
  class ListIndexNotificationBackendEKP {
    ...
    handleItemActions() {
      ...
      _this.tableObj.on('click', '.edit-row', function (e) {
        ...
        //get AJAX data
        Cloud.getNotification.with({ id: id }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
          if (err) {
            console.log(err);
            return;
          }
    ...
    initSweetAlert(id) {
      swal({
        ...
        if (value) {
          //get AJAX data
          Cloud.trashNotification.with({ ids: id }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
            if (err) {
              console.log(err);
              return;
    ...
    initPushAlert(id) {
      swal({
        ...
        if (value) {
          //get AJAX data
          Cloud.pushNotification.with({ id: id }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
            if (err) {
              console.log(err);
              return;
            } else if (responseBody) {

...
module.exports = {
  add: async (req, res) => {
    ...
    if (params.method == 2) {
      ...
      let feeInvoiceObj = await FeeInvoice.findOne({ id: params.feeInvoiceId });
      if (!feeInvoiceObj) return res.badRequest(ErrorMessage.FEE_INVOICE_NOT_FOUND);

      let defaultCurrency = await Currency.findOne({ convertRate: 1 });
      ...
      if (charge && charge.status == 'succeeded') {
        ...
        let dataNotification = {
          title:  sails.__('Fee Invoice %s is paid completely', feeInvoiceObj.title),

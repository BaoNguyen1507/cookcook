...
class ListIndexFeeItemBackendEKP {
  ...
  handleItemActions() {
    ...
    _this.tableObj.on('click', '.edit-row', function (e) {
      ...
      //get AJAX data
      Cloud.getFeeItem.with({ id: id }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
        if (err) {
          console.log(err);
          return;
        }
  ...
  initSweetAlert(id) {
    ...
    swal({
      ...
      if (value) {
        //get AJAX data
        Cloud.deleteFeeItem.with({ ids: id }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
          if (err) {
            console.log(err);
            return;

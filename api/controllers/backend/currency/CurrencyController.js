...
module.exports = {
  add: async (req, res) => {
    ...
    if (!params.convertRate || isNaN(params.convertRate)) return res.badRequest(ErrorMessages.CURRENCY_CONVERT_RATE_REQUIRED);
    ...
    //CHECK DEFAULT CURRENCY IS EXSITED?
    if (params.convertRate == 1) {
      let checkDefault = await Currency.findOne({ convertRate: 1 });
      if (checkDefault) return res.badRequest(ErrorMessages.DEFAULT_CURRENCY_EXISTED);
    }
    ...
    const newData = {
      ...
      convertRate: parseFloat(params.convertRate),
  ...
  edit: async (req, res) => {
    ...
    //CHECK DEFAULT CURRENCY IS EXSITED?
    if (params.convertRate == 1) {
      let checkDefault = await Currency.findOne({ id: {'!=' : params.id}, convertRate: 1 });
      if (checkDefault) return res.badRequest(ErrorMessages.DEFAULT_CURRENCY_EXISTED);
    }
  ...
  search: async (req, res) => {
    ...
    for (let currency of arrObjCurrencys) {
      ...
      tmpData.convertRate = currency.convertRate;
      tmpData.tool = await sails.helpers.renderRowAction(currency);
      ...
      tmpData.status = `
        ...
        if (currency.status == 1 && currency.convertRate == 1) {
          tmpData.status = `
            <label class="switch">
              <input class="switchStatus" type="checkbox" data-id="${currency.id}" checked disabled>
              <span class="slider"></span>
            </label>`;
        } else if (currency.status == 1) {
          tmpData.status = `
            <label class="switch">
  ...
  switchStatus: async (req, res) => {
    ...
    if (currencyObj.convertRate == 1) return res.badRequest(ErrorMessages.DEFAULT_CURRENCY_NOT_ALLOW);

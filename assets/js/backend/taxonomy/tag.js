class IndexTagBackendEKP extends BaseBackendEKP {
  constructor() {
    super();
    this.initialize();
  }
  ...
  initialize() {
    ...
    this.form = new FormIndexTagBackendEKP(this);
    this.list = new ListIndexTagBackendEKP(this);

  }
}

class FormIndexTagBackendEKP {
  constructor(opts) {
    _.extend(this, opts);
    this.formId = 'formTag';
  ...
  initValidation() {
    ...
    _this.formObj.formValidation({
      ...
      .on('success.form.fv', function (e) {
        ...
        e.preventDefault();
        console.log('----- FORM TAG ----- [SUBMIT][START]');
        let $form = $(e.target);
        let formData = $form.serializeArray();
        let tmpData = {};
        _.each(formData, (item) => {
          if (item.name == 'taggroup') {
        ...
        console.log('----- FORM TAG ----- [SUBMIT][END]');
      });
  }
...
class ListIndexTagBackendEKP {
  ...
  initDataTable() {
    ...
    var table = this.tableObj.DataTable({
      ...
      "processing": true,
      "serverSide": true,
      "ajax": "/api/v1/backend/taxonomy/search?status=" + params.status + "&type=tag",
  ...
  initSweetAlert(id) {
    swal({
      ...
      if (value) {
        //get AJAX data
        Cloud.trashTaxonomy.with({ ids: id }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
          if (err) {
            console.log(err);
            return;
  ...
  initSwitchStatus() {
		...
		$(document).ready(function () {
			$('.switchStatus').change(function () {
				let id = $(this).attr('data-id');
        Cloud.switchStatusTaxonomy.with({ id: id }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
					if (err) {
						console.log(err);
						return;
					} else if (responseBody) {

...
module.exports = {
  ...
  edit: async (req, res) => {
    ...
    if (foundEmailUser.length) {
      return res.badRequest(ErrorMessage.USER_ERR_EMAIL_EXISTED);
    } else if (foundPhoneUser.length) {
      return res.badRequest(ErrorMessage.USER_ERR_PHONE_EXISTED);
    }
    ...
    if (!foundOtherUser.length) {
      ...
      return res.badRequest(ErrorMessage.USER_ERR_EMAIL_PHONE_EXISTED);
    }
  },
